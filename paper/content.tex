...
Luyben et. al \citep{luyben1996design} observed a correlation between
degrees of freedom in design-time models and in control models,
especially for reactors and distillation columns. Quantities specified
at design time, such as inlet flow rates or outlet
...
In a design case, the design variables can be replaced by dependent
variables to meet target conditions. In a performance-rating scenario,
the design is fixed. Because the model already defines which variables
are design variables, and records the link between each design variable
and the dependent variable replacing it, switching between design and
performance rating only requires removing the replacements on the design
variables. The model remains fully defined.

Replacing an operating variable with a dependent variable represents the
effect of effective control on that operating variable. Removing those
replacements instead models the effect of changing the operating
variable on the rest of the process. This also maintains a fully defined
model. Variable replacement therefore supports switching between design,
performance-rating, and operation studies with minimal manual changes.
...
convention, all inlet variables are automatically considered Canonical
...
\item
  ...
  are not structurally singular. This can be done by choosing a
  Canonical Variable that is part of the Dulmage-Mendelson Over-constrained set 
...
can be fixed to zero if one of the inlet flows is unfixed as a Canonical
...
Because of the replacement system, it is clear why each of
...
For the pump, the variable replacement pattern is similar to a control
scheme in a P\&ID diagram, where pump work is adjusted from a pressure
measurement. These are replacements of operating variables.
...
another model upstream as Canonical Variables. This is done after the unit models
...
However, in pyomo-replace, the model is already square, and the user can
...
replaced by other variables. As no Canonical Variables have yet been
replaced, it prints a list of all the variables for which a value must
be set, or which must be replaced:
...
Replacing a variable requires calling a function and passing the
Canonical Variable to unfix and the new variable to fix:

\begin{verbatim}
> replace_canonical_var(m.fs.vaporizer.overall_heat_transfer_coefficient,
                    m.fs.vaporizer.cold_side_outlet.enth_mol)
> replace_canonical_var(m.fs.preheater.overall_heat_transfer_coefficient,
                    m.fs.preheater.cold_side_outlet.enth_mol)
> replace_canonical_var(m.fs.recuperator.overall_heat_transfer_coefficient,
                    m.fs.recuperator.cold_side_outlet.enth_mol)
> replace_canonical_var(m.fs.condenser.overall_heat_transfer_coefficient,
                    m.fs.condenser.hot_side_outlet.enth_mol)
> replace_canonical_var(m.fs.pump.work_mechanical, 
                    m.fs.pump.outlet.pressure)
> replace_canonical_var(m.fs.turbine.work_mechanical, 
                    m.fs.turbine.outlet.pressure)
\end{verbatim}

Now we can run \texttt{pprint\_canonical\_replacements} again, to show that
...
To switch this model from a design case to a performance-rating case,
the replacements on the design variables can be removed:
...
but keeps the replacements on the operating variables. This is scalable
to larger models because the design variables are tagged ahead of time.
Because of the coupling between design variables and their replaced variables, the model includes
...
This reverts to the original set of canonical variables. Variable
replacement and the tagging of design and operating variables therefore
provide the metadata required to use the model across multiple use
cases.
...
One advantage of Variable Replacement in a GUI is that it provides
additional context for unit operations. In a GUI, that context can be
shown by
...
A useful comparison comes from programming language theory.
...
We have previously discussed using the car example that some
Canonical Variables are design variables (i.e \(k\)), and some are
...
dependent variables are to changes. In the variable replacement
framework, this can be done by undoing all replacements, as the
decision variables are the Canonical Variables.
...
\caption{Alternative initialisation routine, making use of Canonical
...
Consider this model of a pump connected to a heater, with all the
...
fixed variables are coupled to a set of Canonical Variables, it becomes straightforward to
see when to unfix the variable: when the Canonical Variable it is connected to is
removed.
...
or operating parameters, supports switching between different use cases.
...
different types of EOMs. Emperical studies are also needed to validate the effectiveness of
the approach across a broad range of applications. Nonetheless, these techniques hold promise to
