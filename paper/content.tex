...
data structures. Typically, this is some form grouping variables and 
constraints into a reusable component, such as blocks in Pyomo \citep{hart2011pyomo}, or
models in GPROMS, \citep{amador2019introduction}. 
...
will call them ``Standard Variables''. They are all independent. All other
variables can be defined in terms of these Standard Variables\footnote{This
...
It follows that if all Standard Variables are fixed in a model, then the
...
Standard Variables defined, and then if you want to set something else, you
must choose a state variable to ``replace'', or ``unfix'' in Pyomo
language.

Starting with all the Standard Variables fixed means you never have a
...
the Standard Variables, as you only need to define one way to
...
degrees of freedom. These two variables will become our Standard Variables,
the ones ``fixed by default'' in the model.

Choosing which variables are the Standard Variables is an important step in
...
variables, it is appropriate they are used as the Standard Variables.
...
else to unfix. They must unfix one of the Standard Variables, either \(k\)
...
Pyomo-replace contains methods to keep track of the Standard Variables in
...
Pyomo-replace is a simple library to keep track of the Standard Variables
...
Typically, the set of Standard Variables would be defined by the author of
