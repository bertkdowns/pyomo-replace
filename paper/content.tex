...
This article discusses how treating equation-oriented models more like
explicitly defined systems makes it simpler for a modeller to build,
initialise, and reason about. We propose that the model developer should explicitly define 
which variables in the model are design variables, which are operating variables, and which are dependent variables.
Design and operating variables are fixed by default. As required, design and operating variables can be replaced
by dependent variables in the equation oriented model.

While this approach is usually followed by model developers in practice, it is not formally included in the design
of most equation-oriented modelling frameworks. By formally including it, the AML itself can provide guarantees 
about the behaviour of the system, and it becomes easier to build tools on top of 
the modelling framework to help with model construction and initialisation.


Our library, pyomo-replace, demonstrates some
advantages of this approach in the Pyomo and IDAES
\citep{miller2018next} ecosystem, and the Ahuora Platform provides a
case study of this approach in a graphical application.
...
Similar features are avaliable in other process modelling software such as gPROMS and DWSIM
...
defines a seperate initialisation routine for each unit model that is added \citep{lee2021idaes}. However, as there
...
The implicit formulation of EOMs means there is much freedom in how they are defined.
However, the expressiveness makes it much harder to reason about the structure or 
automate parts of model construction such as initialisation. When multiple EOMs are 
combined using principles of abstraction, the expressiveness of
the model compounds.
In software systems, providing restrictions on what you can do allows you to provide guarantees about
the behaviour of the system. 

We propose that each submodel be defined in a specific way:

\begin{itemize}
  \item a set of `Independent Variables' are fixed by default, such that the submodel has zero degrees of freedom, and all other variables are defined in terms of these Independent Variables. These Independent Variables are either design variables or operating variables in a process model.
  \item if a user wants to fix a variable that is not an Independent Variable, they must choose an Independent Variable to unfix, and replace it with one of the dependent variables they want to fix.
  \item if an Independent Variable is unfixed, an initial guess must be provided for it.
\end{itemize}

In essense, we treat the EOM as if it were explicitly defined. Note that the model itself does not need to be explicitly defined,
but in the user interface level it is treated as if it were.

This leads to some helpful emergent properties that makes modelling large systems easier. 
Degrees of freedom are no longer a problem, because the Independent Variables fully define the model, and for any variable that is fixed, a design or operating variable is unfixed, maintaining the degrees of freedom at zero.
Initialisation becomes easier, as initialisation methods only need to be defined for the Independent Variables. To convert a model from a design to operation to performance rating model, you can simply remove the replacements on the design or operating variables to re-fix them, while maintaining a fully defined model.
...
\(c(\hat{x}) = 0 \; \forall c \in C\) and \(f(\hat{x}) = 0 \; \forall f \in F\).
...
\(f_i\) to \(F\). We define \(F' = F \cup \{f_i\}\). However, if this is
...
\[
F_{new} = (C \backslash \{f_j\}) \cup \{f_i\}
\]
...
As the result of a replacement is still an EOM, the process can be repeated. Each time it is repeated, \(\mathcal{F}_{i}\) is updated to \(\mathcal{F}_{i+1}\).  This gradually replaces the original set of fixed variables (\(\mathcal{F}\), also called the Independent Variables) with a new set of fixed variables (\(\mathcal{F}_{replaced}\)). 
...
Initalisation methods can be written to calculate initial values for all variables based on the initial set of fixed variables (\(\mathcal{F}\)). After the initialisation method is run, the model can be solved with the new set of fixed variables (\(\mathcal{F}_{replaced}\)) to calculate the correct values of any variables that were originally fixed. This allows the same initialisation method to be used regardless of which variables are fixed as long as an initial guess is provided for the Independent Variables.
...
built on IDAES and pyomo, called pyomo-replace. It keeps track of the Independent Variables
in an IDAES model, and what variables are replacing them. The list of
Independent Variables and replacements can be easily printed to the screen to
...
\subsubsection{Independent Variables}\label{state-variables}}

Because each block contains a set of variables and constraints, it can
be considered an independent system of equations.
Thus, each block has its own set of Independent Variables that are fixed by default to make it a square model.

The Independent Variables are chosen by first fixing all inlet variables. By
...
These variables are also considered Independent Variables.

The set of Independent Variables is defined by the author of
the block. This means that if a user imports a block from an external library,
it comes with a predefined set of Independent Variables and initialisation methods that work with them.
...
Independent Variables are fixed, and all unconnected inlets are fixed, and so
...
Independent Variables. For this to happen, a couple of conditions need to be
met:
...
\item
  To prevent the model becoming over-defined, an independent variable must be
  chosen and unfixed. We say the independent variable has been ``replaced''
  with the new variable.
\item
  The Independent Variable must be chosen such that all equations in the model
...
Metadata about which variables are replacing which Independent Variables is
stored, for initialisation and interpretation purposes.
If a variable is ever unfixed, the Independent Variable it replaced
must be fixed again.
...
\texttt{Compressor} unit model that defines the Independent Variables that
...
requires that all inlet ports are registered as Independent Variables:
...
Now we can view all the Independent Variables:

\begin{verbatim}
> pprint_replacements(m.fs)
\end{verbatim}

This prints a list of all Independent Variables, and any that are being
...
\begin{verbatim}
Unreplaced Independent Variables:
...
compressor mechanical work.  Replacing a variable is as simple as calling a function,
passing the new variable to fix and the Independent Variable to unfix:

\begin{verbatim}
> replace_state_var(m.fs.compressor.work_mechanical, m.fs.heater.outlet.pressure)
\end{verbatim}
...
Replaced Independent Variables:
  (Fixed Variable) -> (Replaced Independent Variable)
  fs.h1.outlet.pressure -> fs.compressor.work_mechanical

Unreplaced Independent Variables in block fs:
  fs.compressor.efficiency_isentropic
  fs.compressor.inlet.flow_mol
  fs.compressor.inlet.enth_mol
  fs.compressor.inlet.pressure
...
Independent Variables, and provide a guess for \texttt{ratioP}. This model can
...
directly rather than worrying about which Independent Variable you are
replacing. However,it provides a natural form of documentation for the model and modelling process,
as shown above. It also simplifies initialisation and interpretation of the model, as will
be discussed in \Cref{properties-of-a-variable-replacement-approach-to-equation-oriented-modelling}.
...
Independent Variables. They can be replaced by other variables. This is
...
Additionally, knowing the list of Independent Variables may still provide good
context for optimisation, as any Independent Variables you unfix are the ones
...
To demonstrate the usefulness of a these methods in designing process models,
...
demonstrating which Independent Variables are being replaced.}
\end{figure}

A model of a geothermal plant, outlined in \citep{severinsen2024digital}
is shown here as a test case. This model includes four heat exchangers
...
controller, but is not an Independent Variable.}
\end{figure}
...
Both the valves have two Independent Variables: the valve coefficient, and the
...
treated as additional Independent Variables if it is appropriate. It would be
...
Each unit operation has a set of Independent Variables, where the user can
...
appropriate Independent Variable has to be chosen and ``replaced''. This makes
the Independent Variable a ``guess'': the user still needs to enter a value,
...
up like an Independent Variable, with a message to show which variable it is
...
In the example image, the ``Pressure Drop'' Independent Variable is fixed at 0
kPa. The ``Mechanical Work'' Independent Variable in the Compressor is being
...
styling fixed Independent Variables, guesses, and calculated variables
...
Independent Variables and inlet ports are fixed by default to ensure the
degrees of freedom are still zero. When a unit operation is removed,
anything that is replacing it's Independent Variables is also unfixed, and any
...
specified for the Independent Variables, these can be used in the
...
This method of replacing Independent Variables to define a model does not
...
\item
  ...
  provided for Independent Variables.
\item
...
Using a Variable Replacement approach, a set of Independent Variables would be
...
Independent Variable, an appropriate\footnote{i.e an Independent Variable that would
  be part of the Dulmage-Mendelson over-constrained set if the other
  variable was fixed and nothing was unfixed} Independent Variable must be
...
variables are design Independent Variables (i.e \(k\)), and some are
...
Independent Variables, by calculating a desired \(k\). In an operation case,
...
the decision variables are the Independent Variables.
...
The concept of ``Independent Variables'' can simplify this process, by
...
values or guesses of the Independent Variables. Then, once the model has been
initialised in a sensible location, any Independent Variables that are
...
initial guesses are provided for all the Independent Variables, but this is a
...
maintainability. Technically, it doesn't matter which Independent Variable a
...
Independent Variables already defined by the developer of the Compressor Unit
...
require explicitly replacing the mechanical work Independent Variable. This
...
fixed variables are coupled to specific Independent Variables.}
\end{figure}

Consider this model of a pump connected to a heater, with all the state
variables defined. The inlet conditions are Independent Variables, and so is
...
fixed variables are coupled to a set of Independent Variables, it is easy to
...
If Independent Variables are defined on each block added to a model, those who
...
flowsheet. The coupling between Independent Variables and fixed variables
...
set of Independent Variables and initial guesses for them makes it simpler to
...
its potential benefits, further research is required to systematically
