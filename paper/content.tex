...
\begin{itemize}
  \item a set of `Standard Variables' are fixed by default, such that the submodel has zero degrees of freedom, and all other variables are defined in terms of these Standard Variables. These Standard Variables are either design variables or operating variables in a process model.
  \item if a user wants to fix a variable that is not a Standard Variable, they must choose a Standard Variable to unfix, and replace it with one of the dependent variables they want to fix.
  \item if a Standard Variable is unfixed, an initial guess must be provided for it.
\end{itemize}
...
Degrees of freedom are no longer a problem, because the Standard Variables fully define the model, and for any variable that is fixed, a design or operating variable is unfixed, maintaining the degrees of freedom at zero.
Initialisation becomes easier, as initialisation methods only need to be defined for the Standard Variables. To convert a model from a design to operation to performance rating model, you can simply remove the replacements on the design or operating variables to re-fix them, while maintaining a fully defined model.
...
As the result of a replacement is still an EOM, the process can be repeated. Each time it is repeated, \(\mathcal{F}_{i}\) is updated to \(\mathcal{F}_{i+1}\).  This gradually replaces the original set of fixed variables (\(\mathcal{F}\), also called the Standard Variables) with a new set of fixed variables (\(\mathcal{F}_{replaced}\)). 
...
built on IDAES and pyomo, called pyomo-replace. It keeps track of the Standard Variables
in an IDAES model, and what variables are replacing them. The list of
Standard Variables and replacements can be easily printed to the screen to
...
\subsubsection{Standard Variables}\label{state-variables}}

Because each block contains a set of variables and constraints, it can
be considered an independent system of equations.
Thus, each block has its own set of Standard Variables that are fixed by default to make it a square model.

The Standard Variables are chosen by first fixing all inlet variables. By
...
These variables are also considered Standard Variables.

The set of Standard Variables is defined by the author of
the block. This means that if a user imports a block from an external library,
it comes with a predefined set of Standard Variables and initialisation methods that work with them.
...
Standard Variables are fixed, and all unconnected inlets are fixed, and so
...
Standard Variables. For this to happen, a couple of conditions need to be
met:
...
Metadata about which variables are replacing which Standard Variables is
stored, for initialisation and interpretation purposes.
If a variable is ever unfixed, the state variable it replaced
must be fixed again.
...
\texttt{Compressor} unit model that defines the Standard Variables that
...
requires that all inlet ports are registered as Standard Variables:
...
Now we can view all the Standard Variables:

\begin{verbatim}
> pprint_replacements(m.fs)
\end{verbatim}

This prints a list of all Standard Variables, and any that are being
...
Standard Variables, and provide a guess for \texttt{ratioP}. This model can
...
Standard Variables. They can be replaced by other variables. This is
...
Additionally, knowing the list of Standard Variables may still provide good
context for optimisation, as any Standard Variables you unfix are the ones
...
demonstrating which Standard Variables are being replaced.}
\end{figure}
...
Both the valves have two Standard Variables: the valve coefficient, and the
...
treated as additional Standard Variables if it is appropriate. It would be
...
Each unit operation has a set of Standard Variables, where the user can
...
styling fixed Standard Variables, guesses, and calculated variables
...
Standard Variables and inlet ports are fixed by default to ensure the
degrees of freedom are still zero. When a unit operation is removed,
anything that is replacing it's Standard Variables is also unfixed, and any
...
specified for the Standard Variables, these can be used in the
...
This method of replacing Standard Variables to define a model does not
...
\item
  ...
  provided for Standard Variables.
\item
...
Using a Variable Replacement approach, a set of Standard Variables would be
...
variables are design Standard Variables (i.e \(k\)), and some are
...
Standard Variables, by calculating a desired \(k\). In an operation case,
...
the decision variables are the Standard Variables.
...
The concept of ``Standard Variables'' can simplify this process, by
...
values or guesses of the Standard Variables. Then, once the model has been
initialised in a sensible location, any Standard Variables that are
...
initial guesses are provided for all the Standard Variables, but this is a
...
Standard Variables already defined by the developer of the Compressor Unit
...
fixed variables are coupled to specific Standard Variables.}
\end{figure}

Consider this model of a pump connected to a heater, with all the state
variables defined. The inlet conditions are Standard Variables, and so is
...
fixed variables are coupled to a set of Standard Variables, it is easy to
...
If Standard Variables are defined on each block added to a model, those who
...
flowsheet. The coupling between Standard Variables and fixed variables
...
set of Standard Variables and initial guesses for them makes it simpler to
