...
the unit models are imported from the pyomo-replace library, which have already defined the Initial Variables 
...
set values for the Initial Variables, or replace them with other variables as required.

Pyomo-replace provides a method to print the list of Initial Variables and any replacements that have been made:
...
Inlet variables are tagged seperately as they don't fit neatly into either category. 
Additional categories of variables can be added if required.

We can then choose to replace any of the Initial Variables with other variables in the model.
...
passing the Initial Variable to unfix and the new variable to fix:
...
Then a value can be specified for all the variables that are replacing Initial Variables, and all the Initial Variables that
...
In \cref{fig:ahuora-replacement}, the ``Isentropic Efficiency'' Initial Variable is fixed at approximately 70\%. 
The ``Mechanical Work'' Initial Variable in the Compressor is being
...
the mechanical work Initial Variable with the outlet pressure. This
...
the relationships between variables to the forefront. Tagging Initial Variables
...
Additionally, enforcing a standard set of Initial Variables and requiring the user
