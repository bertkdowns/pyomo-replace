...
their surroundings \citep{walmsley2024adaptive}. As these models have
become larger, being able to understand all of the constraints within
the system simultaneously becomes harder and harder, making it harder to
...
initialise, and reason about. We propose that the model developer should explicitly define an initial set
of variables to constrain. As required, explicit variables can be replaced
by other variables in the equation oriented model.
...
required in a mathematical model. However, in AMLs additional
...
programming languages, which make it easier to build additional tools 
on top of the modelling language to help with model construction and
initialisation.

\citep{jusevivcius2021experimental}.
...
much uptake in systems such as Pyomo. Additionally, Dulmage-Mendelson
Decomposition provides a big list of over-constrained or
under-constrained components, but does not provide any insight into
...
\begin{itemize}
  \item a set of `Standard Variables' are fixed by default, such that the submodel has zero degrees of freedom, and all other variables are defined in terms of these Standard Variables.
  \item if a user wants to fix a variable that is not a Standard Variable, they must choose a Standard Variable to unfix, and replace it with one of the calculated variables they want to fix.
...
Degrees of freedom are no longer a problem, because the Standard Variables fully define the model, and for any variable that is fixed, a Standard Variable is unfixed, maintaining the degrees of freedom at zero.
Initialisation becomes easier, as initialisation methods only need to be defined for the Standard Variables.
