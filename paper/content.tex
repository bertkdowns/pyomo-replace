...
\begin{itemize}
  \item a set of `Initial Variables' are fixed by default, such that the submodel has zero degrees of freedom, and all other variables are defined in terms of these Initial Variables. These Initial Variables are either design variables or operating variables in a process model.
  \item if a user wants to fix a variable that is not an Initial Variable, they must choose an Initial Variable to unfix, and replace it with one of the dependent variables they want to fix.
  \item if an Initial Variable is unfixed, an initial guess must be provided for it.
\end{itemize}
...
Degrees of freedom are no longer a problem, because the Initial Variables fully define the model, and for any variable that is fixed, a design or operating variable is unfixed, maintaining the degrees of freedom at zero.
Initialisation becomes easier, as initialisation methods only need to be defined for the Initial Variables. To convert a model from a design to operation to performance rating model, you can simply remove the replacements on the design or operating variables to re-fix them, while maintaining a fully defined model.
...
As the result of a replacement is still an EOM, the process can be repeated. Each time it is repeated, \(\mathcal{F}_{i}\) is updated to \(\mathcal{F}_{i+1}\).  This gradually replaces the original set of fixed variables (\(\mathcal{F}\), also called the Initial Variables) with a new set of fixed variables (\(\mathcal{F}_{replaced}\)). 
...
built on IDAES and pyomo, called pyomo-replace. It keeps track of the Initial Variables
in an IDAES model, and what variables are replacing them. The list of
Initial Variables and replacements can be easily printed to the screen to
...
\subsubsection{Initial Variables}\label{state-variables}}

Because each block contains a set of variables and constraints, it can
be considered an independent system of equations.
Thus, each block has its own set of Initial Variables that are fixed by default to make it a square model.

The Initial Variables are chosen by first fixing all inlet variables. By
...
These variables are also considered Initial Variables.

The set of Initial Variables is defined by the author of
the block. This means that if a user imports a block from an external library,
it comes with a predefined set of Initial Variables and initialisation methods that work with them.
...
Initial Variables are fixed, and all unconnected inlets are fixed, and so
...
Initial Variables. For this to happen, a couple of conditions need to be
met:
...
\item
  To prevent the model becoming over-defined, an Initial Variable must be
  chosen and unfixed. We say the Initial Variable has been ``replaced''
  with the new variable.
\item
  The Initial Variable must be chosen such that all equations in the model
...
Metadata about which variables are replacing which Initial Variables is
stored, for initialisation and interpretation purposes.
If a variable is ever unfixed, the Initial Variable it replaced
must be fixed again.
...
\texttt{Compressor} unit model that defines the Initial Variables that
...
requires that all inlet ports are registered as Initial Variables:
...
Now we can view all the Initial Variables:

\begin{verbatim}
> pprint_replacements(m.fs)
\end{verbatim}

This prints a list of all Initial Variables, and any that are being
...
passing the new variable to fix and the Initial Variable to unfix:
...
Replaced Initial Variables:
  (Fixed Variable) -> (Replaced Initial Variable)
  fs.h1.outlet.pressure -> fs.compressor.work_mechanical

Unreplaced Initial Variables in block fs:
  fs.compressor.efficiency_isentropic
  fs.compressor.inlet.flow_mol
  fs.compressor.inlet.enth_mol
  fs.compressor.inlet.pressure
...
Initial Variables, and provide a guess for \texttt{ratioP}. This model can
...
directly rather than worrying about which Initial Variable you are
...
Initial Variables. They can be replaced by other variables. This is
...
Additionally, knowing the list of Initial Variables may still provide good
context for optimisation, as any Initial Variables you unfix are the ones
...
demonstrating which Initial Variables are being replaced.}
\end{figure}
...
controller, but is not an Initial Variable.}
\end{figure}
...
Both the valves have two Initial Variables: the valve coefficient, and the
...
treated as additional Initial Variables if it is appropriate. It would be
...
Each unit operation has a set of Initial Variables, where the user can
...
appropriate Initial Variable has to be chosen and ``replaced''. This makes
the Initial Variable a ``guess'': the user still needs to enter a value,
...
up like an Initial Variable, with a message to show which variable it is
...
In the example image, the ``Pressure Drop'' Initial Variable is fixed at 0
kPa. The ``Mechanical Work'' Initial Variable in the Compressor is being
...
styling fixed Initial Variables, guesses, and calculated variables
...
Initial Variables and inlet ports are fixed by default to ensure the
degrees of freedom are still zero. When a unit operation is removed,
anything that is replacing it's Initial Variables is also unfixed, and any
...
specified for the Initial Variables, these can be used in the
...
This method of replacing Initial Variables to define a model does not
...
\item
  ...
  provided for Initial Variables.
\item
...
Using a Variable Replacement approach, a set of Initial Variables would be
...
Initial Variable, an appropriate\footnote{i.e an Initial Variable that would
  be part of the Dulmage-Mendelson over-constrained set if the other
  variable was fixed and nothing was unfixed} Initial Variable must be
...
variables are design Initial Variables (i.e \(k\)), and some are
...
Initial Variables, by calculating a desired \(k\). In an operation case,
...
the decision variables are the Initial Variables.
...
The concept of ``Initial Variables'' can simplify this process, by
...
values or guesses of the Initial Variables. Then, once the model has been
initialised in a sensible location, any Initial Variables that are
...
initial guesses are provided for all the Initial Variables, but this is a
...
maintainability. Technically, it doesn't matter which Initial Variable a
...
Initial Variables already defined by the developer of the Compressor Unit
...
require explicitly replacing the mechanical work Initial Variable. This
...
fixed variables are coupled to specific Initial Variables.}
\end{figure}

Consider this model of a pump connected to a heater, with all the state
variables defined. The inlet conditions are Initial Variables, and so is
...
fixed variables are coupled to a set of Initial Variables, it is easy to
...
If Initial Variables are defined on each block added to a model, those who
...
flowsheet. The coupling between Initial Variables and fixed variables
...
set of Initial Variables and initial guesses for them makes it simpler to
