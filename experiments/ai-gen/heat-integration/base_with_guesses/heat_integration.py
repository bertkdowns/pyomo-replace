...
def specify_model(model: pyo.ConcreteModel) -> None:
    ...
    fs.cooling_tower_1.heat_duty.fix(100.0e3)
    fs.cooling_tower_1.deltaP.fix(0.0)
    fs.cooling_tower_2.heat_duty.fix(100.0e3)
    fs.cooling_tower_2.deltaP.fix(0.0)

    fs.air_source_heat_pump.source_inlet.flow_mol.fix(1000.0)
    fs.air_source_heat_pump.source_inlet.pressure.fix(100.0e3)
    fs.air_source_heat_pump.source_inlet.enth_mol.fix(
        pyo.value(water.htpx(T=298.15 * pyo.units.K, p=100.0e3 * pyo.units.Pa)),
    )
    fs.air_source_heat_pump.work_mechanical.fix(100.0e3)
    fs.air_source_heat_pump.coefficient_of_performance.unfix()
    fs.air_source_heat_pump.efficiency.fix(0.5)
    ...
    fs.steam_demand_splitter.inlet.flow_mol.fix(23.128530015022072)
    fs.steam_demand_splitter.inlet.pressure.fix(1000.0e3)
    fs.steam_demand_splitter.inlet.enth_mol.fix(13349.6505)
    fs.steam_heat_pump.sink.properties_out[TIME].enth_mol.fix(
        pyo.value(water.htpx(p=1000.0e3 * pyo.units.Pa, x=1.0)),
    )

    fs.gas_boiler.deltaP.fix(0.0)
    fs.gas_boiler.control_volume.properties_out[TIME].enth_mol.fix(
        pyo.value(water.htpx(p=1000.0e3 * pyo.units.Pa, x=1.0)),
    )

    # Provide feasible-scale states to the unit-by-unit initialization sequence.
    fs.cooling_tower_1.outlet.flow_mol[TIME].set_value(265.0)
    fs.cooling_tower_1.outlet.pressure[TIME].set_value(100.0e3)
    fs.cooling_tower_1.outlet.enth_mol[TIME].set_value(1500.0)
    fs.cooling_tower_2.outlet.flow_mol[TIME].set_value(265.0)
    fs.cooling_tower_2.outlet.pressure[TIME].set_value(100.0e3)
    fs.cooling_tower_2.outlet.enth_mol[TIME].set_value(1500.0)
    fs.air_source_heat_pump.source_outlet.flow_mol[TIME].set_value(1000.0)
    fs.air_source_heat_pump.source_outlet.pressure[TIME].set_value(100.0e3)
    fs.air_source_heat_pump.source_outlet.enth_mol[TIME].set_value(1674.3)
    fs.air_source_heat_pump.sink_outlet.flow_mol[TIME].set_value(103.0)
    fs.air_source_heat_pump.sink_outlet.pressure[TIME].set_value(100.0e3)
    fs.air_source_heat_pump.sink_outlet.enth_mol[TIME].set_value(4208.8)
    fs.air_source_heat_pump.heat_duty[TIME].set_value(316.0e3)
    fs.air_source_heat_pump.source.heat[TIME].set_value(-216.0e3)
    fs.air_source_heat_pump.coefficient_of_performance.set_value(3.16)
    fs.air_source_heat_pump.delta_temperature_lift[TIME].set_value(50.0)
    fs.steam_demand_splitter.split_fraction[TIME, "steam_heat_pump"].set_value(0.85)
    fs.steam_demand_splitter.split_fraction[TIME, "gas_boiler"].set_value(0.15)
    fs.steam_heat_pump.source_outlet.flow_mol[TIME].set_value(634.0)
    fs.steam_heat_pump.source_outlet.pressure[TIME].set_value(100.0e3)
    fs.steam_heat_pump.source_outlet.enth_mol[TIME].set_value(1590.0)
    fs.steam_heat_pump.work_mechanical[TIME].set_value(492.0e3)
    fs.steam_heat_pump.coefficient_of_performance.set_value(1.46)
    fs.steam_heat_pump.delta_temperature_lift[TIME].set_value(169.0)
    fs.steam_heat_pump.source.heat[TIME].set_value(-228.0e3)
    fs.steam_heat_pump.sink_inlet.flow_mol[TIME].set_value(19.6)
    fs.steam_heat_pump.sink_inlet.pressure[TIME].set_value(1000.0e3)
    fs.steam_heat_pump.sink_inlet.enth_mol[TIME].set_value(13350.0)
    fs.steam_heat_pump.sink_outlet.flow_mol[TIME].set_value(19.6)
    fs.steam_heat_pump.sink_outlet.pressure[TIME].set_value(1000.0e3)
    fs.gas_boiler.inlet.flow_mol[TIME].set_value(3.5)
    fs.gas_boiler.inlet.pressure[TIME].set_value(1000.0e3)
    fs.gas_boiler.inlet.enth_mol[TIME].set_value(13350.0)
    fs.gas_boiler.heat_duty[TIME].set_value(128.0e3)

    steam_heat_pump_initialize = fs.steam_heat_pump.initialize

    def initialize_steam_heat_pump(*args, **kwargs):
        """Let the heat-pump initializer seed its source heat with the correct sign."""
        fs.steam_heat_pump.heat_duty.unfix()
        try:
            return steam_heat_pump_initialize(*args, **kwargs)
        finally:
            fs.steam_heat_pump.heat_duty.fix(720.0e3)

    fs.steam_heat_pump.initialize = initialize_steam_heat_pump
