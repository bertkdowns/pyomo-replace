...
def specify_model(model: pyo.ConcreteModel) -> None:
    ...
    for exchanger, coefficient, area in (
        (fs.E1HX, 1000.0, 10.0),
        (fs.E2HX, 1000.0, 1.0),
        (fs.HeatExchanger3, 70.0, 1.0),
    ):
        specs.fix(exchanger.overall_heat_transfer_coefficient, coefficient)
        specs.fix(exchanger.area, area)
        specs.fix(exchanger.hot_side.deltaP, 0.0)
        specs.fix(exchanger.cold_side.deltaP, 0.0)

    for separator in (fs.E1PS, fs.E2PS, fs.PhaseSeparator3):
        specs.fix(separator.split_fraction[TIME, "outlet_1", "Liq"], 0.99)
        specs.fix(separator.split_fraction[TIME, "outlet_1", "Vap"], 0.01)
